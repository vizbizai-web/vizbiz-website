import crypto from 'crypto';

const sa = {
  private_key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
  client_email: 'gsc-reader@vizbiz-gsc.iam.gserviceaccount.com',
  token_uri: 'https://oauth2.googleapis.com/token',
};

async function getToken() {
  const header = { alg: 'RS256', typ: 'JWT' };
  const now = Math.floor(Date.now() / 1000);
  const claim = { iss: sa.client_email, scope: 'https://www.googleapis.com/auth/webmasters https://www.googleapis.com/auth/siteverification', aud: sa.token_uri, iat: now, exp: now + 3600 };
  const b64 = o => Buffer.from(JSON.stringify(o)).toString('base64url');
  const si = b64(header) + '.' + b64(claim);
  const sig = crypto.createSign('RSA-SHA256').update(si).sign(sa.private_key, 'base64url');
  const jwt = si + '.' + sig;
  const r = await fetch(sa.token_uri, { method: 'POST', headers: {'Content-Type':'application/x-www-form-urlencoded'}, body: 'grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer&assertion=' + jwt });
  const { access_token } = await r.json();
  return access_token;
}

const token = await getToken();
console.log('✅ Got token');

// Step 1: Request a meta tag verification token
const vr = await fetch('https://www.googleapis.com/siteVerification/v1/token', {
  method: 'POST',
  headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
  body: JSON.stringify({ verificationMethod: 'FILE', site: { type: 'SITE', identifier: 'https://vizbiz.ai/' } })
});
console.log('Token request:', vr.status);
const vdata = await vr.json();
console.log(JSON.stringify(vdata, null, 2));
